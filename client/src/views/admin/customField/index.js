...
const CustomField = () => {
  ...
  const handleHeadigsCheckboxChange = (event, value) => {
    if (event?.target?.checked) {
      ...
      setSelectedHeadings((prevSelectedHeadings) =>
        prevSelectedHeadings?.filter(
          (selectedHeading) => selectedHeading !== value,
        ),
      );
    }
  };
  ...
  const handleHeadingsDragEnd = async (result) => {
    ...
    const response = await putApi(
      `api/custom-field/change-headings/${moduleId}`,
      newData,
    );
    fetchData();
  };
  ...
  const fetchData = async () => {
    ...
    if (moduleName) {
      ...
      const filterData = response?.data[0]?.fields?.filter(
        (field) =>
          (headingId ? headingId : field?.belongsTo) === field?.belongsTo,
      );
    ...
    if (method === "one") {
      try {
        ...
        const response = await deleteApi(
          `api/custom-field/delete/${id}?moduleId=`,
          moduleId,
        );
  ...
  const handleDeleteHeading = async (id, headingsIds) => {
    if (method === "one") {
      try {
        setIsLoding(true);
        const response = await deleteApi(
          `api/custom-field/delete-heading/${id}?moduleId=`,
          moduleId,
        );
        if (response?.status === 200) {
      ...
      try {
        ...
        let response = await deleteManyApi(
          "api/custom-field/deleteMany-headings",
          payload,
        );
        if (response?.status === 200) {
          setSelectedHeadings([]);
  ...
  return (
    <>
          ...
          <Flex alignItems="center">
            ...
            {selectedHeadings?.length > 0 && (
              <Button
                color="red"
                ml="2"
                onClick={() => handleOpenDeleteModel("", "many")}
                size="sm"
              >
                <DeleteIcon />
              </Button>
          ...
          <Box>
            <Flex>
              ...
              {data?.length > 0 && (
                <Button
                  ml={2}
                  onClick={() => setAddHeadingModel(true)}
                  leftIcon={<AddIcon />}
                  variant="brand"
                  size="sm"
                >
                                  ...
                                  >
                                    ...
                                    {!item?.fixed && (
                                      <Checkbox
                                        ...
                                        isChecked={selectedHeadings?.includes(
                                          item?._id,
                                        )}
                                        onChange={(event) =>
                                          handleHeadigsCheckboxChange(
                                            event,
                                            item?._id,
                                          )
                                        }
      ...
      {data?.length > 0 && (
        <Card mt="5">
          <Flex justifyContent={"space-between"} alignItems={"center"} mt="5">
            ...
            <Box>
              {!isLoading && data[0]?.headings?.length > 0 && (
                <Menu>
                  <MenuButton
                    as={Button}
                    size="sm"
      ...
      <Addfield
        isOpen={addFieldModel}
        onClose={setAddFieldModel}
        moduleName={moduleName}
        field={data[0]?.fields}
        validations={validations}
        moduleId={data[0]?._id}
        fetchData={fetchData}
        headingsData={data?.[0]?.headings}
      />
      <EditField
        isOpen={editModal}
        ...
        validations={validations}
        fetchData={fetchData}
        updateFiled={updateField}
        headingsData={data?.[0]?.headings}
      />

      <AddEditHeading
        isOpen={addHeadingModel || editHeadingModal}
        onClose={editHeadingModal ? setEditHeadingModal : setAddHeadingModel}
        moduleName={moduleName}
        moduleId={data[0]?._id}
        fetchData={fetchData}
        updateData={updateHeading}
        setUpdateData={setUpdateHeading}
      />

      <CommonDeleteModel
        isOpen={deleteMany}
        onClose={() => setDeleteMany(false)}
        type={method === "one" ? "Leads Filed" : "Leads Fileds"}
        handleDeleteData={handleDeleteFiled}
        ids={selectedId}
        selectedValues={selectedValues}
      />
      <CommonDeleteModel
        isOpen={deleteManyHeadings}
        onClose={() => setDeleteManyHeadings(false)}
        type={method === "one" ? "Leads Heading" : "Leads Headings"}
        handleDeleteData={handleDeleteHeading}
        ids={selectedId}
        selectedValues={selectedHeadings}
      />
    </>
  );
};
